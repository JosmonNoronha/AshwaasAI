...
!pip install -q fastapi uvicorn soundfile pyngrok
...
def _decode_audio(audio_bytes: bytes) -> np.ndarray:
    """Decode WAV or M4A bytes → float32 numpy at 16kHz."""
    import soundfile as sf
    try:
        audio, sr = sf.read(io.BytesIO(audio_bytes))
    except Exception:
        from pydub import AudioSegment
        seg = AudioSegment.from_file(io.BytesIO(audio_bytes)).set_channels(1).set_frame_rate(16000)
...
def _transcribe_array(audio: np.ndarray) -> str:
    """Run Whisper on an already-decoded float32 array."""
    max_val = np.abs(audio).max()
    if max_val < 1e-4:
        return ""
    audio = audio / max_val
    segments, _ = whisper_model.transcribe(
        audio, language="mr", beam_size=3, temperature=0.0,
        vad_filter=True, vad_parameters=dict(threshold=0.2, min_speech_duration_ms=200),
    )
...
async def chat(req: ChatRequest):
    ...
    if req.audio_b64:
        audio         = _decode_audio(base64.b64decode(req.audio_b64))
        konkani_input = await asyncio.to_thread(_transcribe_array, audio)
        if not konkani_input:
            raise HTTPException(422, "No speech detected")
    elif req.text:
        konkani_input = req.text
    else:
        raise HTTPException(422, "Provide audio_b64 or text")

    memory = load_session_memory()
    result = await asyncio.to_thread(run_pipeline, konkani_input, memory)
    ...
    return {
        "user_konkani":      konkani_input,
        "user_english":      result["english_candidates"][0],
        "assistant_english": result["english_response"],
        "assistant_konkani": result["konkani_response"],
...
async def end_session():
    import asyncio
    with _session_lock:
        turns_copy = list(_session_turns)
    summary = await asyncio.to_thread(summarize_session, turns_copy)
    memory = load_session_memory()
    memory["summary"] = summary
    memory["turns"]   = turns_copy
...
# Start uvicorn in a background thread so the notebook cell doesn't block
def _run_server():
...
# ── Expose via ngrok (uncomment when running on Kaggle) ─────────────
